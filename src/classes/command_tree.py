...
class CommandTree(app_commands.CommandTree):
    ...
    def _update_app_commands(self, cmds: list[app_commands.AppCommand]) -> dict[str, app_commands.AppCommand]:
    ...
    async def sync(self, *, guild: Snowflake | None = None) -> list[app_commands.AppCommand]:
        ...
        logging.debug(f"app commands synced & app commands list updated locally ({len(cmds)})")
        return cmds
    ...
    async def update_app_commands(self) -> list[app_commands.AppCommand]:
        ...
        self._update_app_commands(cmds)
        logging.debug(f"app commands fetched & updated locally ({len(self.all_app_commands)})")
        return cmds
    ...
    def slash_mention(self, qualified_command_name: str) -> str:
        ...
        return f"</{qualified_command_name}:{self.all_app_commands[parent].id}>" if self.all_app_commands.get(parent) else f"`/{qualified_command_name}`"
